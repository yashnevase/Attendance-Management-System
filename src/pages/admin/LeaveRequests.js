    ...
    const handleAction = async () => {
        ...
        try {
            ...
            const response = await api.put(
                `/leave/override/${selectedLeave.id}`,
                { status:endpoint },
                { headers: { Authorization: `Bearer ${token}` } }
            );
        ...
        <Box sx={{ p: 3 }}>
            ...
            <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    {selectedLeave && (
                        <Box sx={{ mt: 2 }}>
                            <Typography variant="subtitle2" gutterBottom>Employee: {selectedLeave?.name || selectedLeave?.username}</Typography>
                            <Typography variant="subtitle2" gutterBottom>Leave Type: {selectedLeave?.leave_type}</Typography>
                            <Typography variant="subtitle2" gutterBottom>
                                Duration: {new Date(selectedLeave.start_date).toLocaleDateString()} - {new Date(selectedLeave.end_date).toLocaleDateString()}
                            </Typography>
